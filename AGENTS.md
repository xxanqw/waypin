...
- `src/shortcut.rs` (feature-gated) vendors `protocols/hyprland-global-shortcuts-v1.xml` and uses `wayland_scanner` proc-macros. The shortcut `app_id:id` is `waypin:toggle`; users must add `hl.bind("SUPER + V", hl.dsp.global("waypin:toggle"))` to `hyprland.lua` — the protocol does not let the client pick the keybinding.
